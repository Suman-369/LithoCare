...
export default function AboutSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // 1. Initial Hero Reveal (triggered by scroll instead of page load since it's now a section)
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: containerRef.current,
          start: "top 60%", // Start revealing when the video hero section is partially in view
        }
      });
      ...
      // 2. ScrollTrigger Video Section Parallax
  ...
  return (
    <div ref={containerRef} className="bg-white">
      ...
      <section className="relative h-screen w-full overflow-hidden flex items-center justify-center bg-black">
        ...
        >
          ...
          <h1 className="text-5xl md:text-7xl lg:text-[5.5rem] font-bold leading-[1.1] tracking-tight mb-8 font-sans overflow-hidden">
            <span ref={el => { headingLinesRef.current[0] = el }} className="block">Moving Forward.</span>
            <span ref={el => { headingLinesRef.current[1] = el }} className="block text-gray-200">Designed Differently.</span>
          </h1>
          ...
          >
            Experience a new generation of mobility where intelligent engineering, refined design, and everyday performance come together.
          </p>
      ...
      <section className="pt-12 pb-20 md:pt-16 md:pb-24 px-6 lg:px-12 bg-white text-black relative overflow-hidden">
        <div ref={storySectionRef} className="max-w-[1000px] mx-auto text-center flex flex-col items-center">
          {/* Decorative accent */}
          ...
          <h2 className="text-5xl md:text-6xl lg:text-[4.5rem] font-serif font-medium leading-[1.15] mb-16 tracking-tight text-gray-900">
            <span className="block overflow-hidden pb-2"><span className="block story-heading-text">More Than A Vehicle.</span></span>
            <span className="block overflow-hidden pb-2"><span className="block story-heading-text text-gray-400 italic">A New Way To Move.</span></span>
          </h2>
          
          <div className="space-y-8 text-xl md:text-2xl text-gray-500 leading-relaxed max-w-3xl mx-auto font-light">
            <p className="story-paragraph">
              We believe that movement should be effortless, sustainable, and beautifully designed. Our vision started with a simple question: <strong className="font-medium text-gray-900">How can we make urban commuting something people actually look forward to?</strong>
            </p>
            <p className="story-paragraph">
              Through relentless innovation, aerospace-grade materials, and a rider-first design philosophy, we've stripped away the unnecessary to create electric vehicles that are as intuitive as they are powerful.
            </p>
          </div>
        </div>
      </section>
      ...
      {/* Marquee Section */}
      <section ref={marqueeSectionRef} className="bg-[#ccff00] py-3 md:py-4 border-y-[3px] border-black overflow-hidden flex w-full relative z-20">
        <div ref={marqueeInnerRef} className="flex w-max">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="flex shrink-0 items-center whitespace-nowrap" aria-hidden={i > 0 ? "true" : undefined}>
              ...
              <div className="relative w-24 h-10 md:w-48 md:h-16 rounded-full overflow-hidden mx-2 md:mx-4 shrink-0 border-[3px] border-black bg-white">
                <Image src="/images/b1.png" alt="Scooter view" fill className="object-contain p-1 md:p-2" />
              </div>
              ...
              <div className="relative w-24 h-10 md:w-48 md:h-16 rounded-full overflow-hidden mx-2 md:mx-4 shrink-0 border-[3px] border-black bg-white">
                <Image src="/images/b2.png" alt="Scooter view" fill className="object-contain p-1 md:p-2" />
              </div>
              ...
              <div className="relative w-24 h-10 md:w-48 md:h-16 rounded-full overflow-hidden mx-2 md:mx-4 shrink-0 border-[3px] border-black bg-white">
                <Image src="/images/image1.png" alt="Scooter view" fill className="object-contain p-1 md:p-2" />
              </div>
            </div>
          ))}
        </div>
      ...
      >
        ...
        <div className="relative z-10 text-center px-6">
          <h2 className="text-4xl md:text-6xl font-bold tracking-widest text-white uppercase opacity-90 drop-shadow-2xl">
            The Future Is<br />Already Moving.
          </h2>
        </div>
      </section>
